...
import { fetchProductDetailGuest } from '../../../../redux/productManagement/productSliceForGuest/ProductForGuestSlice';
const ProductDetailGuest = () => {
    const dispatch = useDispatch();
    const { id } = useParams();
    const productApi = useSelector((product) => product.guest.detail)
    useEffect(() => {
        window.scrollTo(0, 0)
        dispatch(fetchProductDetailGuest({ id })).then(() => { console.log(productApi) });
      }, [dispatch, id]);
