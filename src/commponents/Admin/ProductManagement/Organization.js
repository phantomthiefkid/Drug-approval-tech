        ...
        <div className='mt-20 mb-20'>
            <div className='py-8 flex px-48'>
                <h1 className='italic text-3xl font-extrabold text-blue-900 flex'>
                    <span className='ml-2'>Chọn tổ chức</span>
                </h1>
            </div>
            <div className='w-5/6 mx-auto mt-20 flex gap-12 bg-opacity-40'>
                {organizations.map((organization, index) => (
                    <Link key={index} to={`/productlist`} className='w-1/3 h-52 relative rounded-xl transition-transform duration-300 transform hover:scale-110 hover:shadow-2xl' style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), url(${organization.image})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
